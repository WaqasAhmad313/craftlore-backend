import { ThemeModel, PageContentModel, PageMetaModel, TeamMemberModel } from './model.ts';
...
export class ThemeService {
  ...
  static async updateTheme(input: UpdateThemeInput): Promise<GlobalTheme> {
    ...
    if (input.theme_name) {
      const nameExists = await ThemeModel.existsByName(input.theme_name, input.id);
    ...
    // Validate theme structure ONLY if provided (partial updates allowed)
    if (input.colors) {
      this.validateThemeColors(input.colors);
    }
  ...
  private static validateThemeColors(colors: unknown): void {
    ...
    const required = ['primary', 'secondary', 'accent', 'neutral', 'success', 'warning', 'error', 'info'];
