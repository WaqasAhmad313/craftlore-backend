...
export class CarbonFootprintController {
   ...
   * UPSERT baseline for a GI product
   */
  static async upsertBaseline(req: Request, res: Response): Promise<void> {
    try {
      const idParam = req.params.id;
      if (!idParam) {
        res.status(400).json({ success: false, message: 'GI Product ID is required' });
        return;
      }
      
      const gi_product_id = parseInt(idParam);
      if (isNaN(gi_product_id)) { 
        res.status(400).json({ success: false, message: 'Invalid ID' }); 
        return; 
      }
      
      const baseline = await CarbonFootprintModel.upsertBaseline({
        ...req.body,
        gi_product_id,
        calculated_by: (req as any).user?.id,
      });
      
      const message = req.body.id ? 'Baseline updated successfully' : 'Baseline created successfully';
      const statusCode = req.body.id ? 200 : 201;
      
      res.status(statusCode).json({ success: true, message, data: baseline });
    } catch (error: any) {
      if (error.message === 'Baseline not found') res.status(404).json({ success: false, message: error.message });
      else res.status(500).json({ success: false, message: error.message });
    }
  }
   ...
   * Activate a baseline
   */
  static async activateBaseline(req: Request, res: Response): Promise<void> {
    try {
      const idParam = req.params.baseline_id;
      if (!idParam) {
        res.status(400).json({ success: false, message: 'Baseline ID is required' });
        return;
      }
      ...
      await CarbonFootprintModel.setBaselineActive(id);
      res.status(200).json({ success: true, message: 'Baseline activated' });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
   ...
   * Delete a baseline
   */
  static async deleteBaseline(req: Request, res: Response): Promise<void> {
    try {
      const idParam = req.params.baseline_id;
      if (!idParam) {
        res.status(400).json({ success: false, message: 'Baseline ID is required' });
      ...
      await CarbonFootprintModel.deleteBaseline(id);
      res.status(200).json({ success: true, message: 'Baseline deleted' });
    } catch (error: any) {
      if (error.message === 'Baseline not found') res.status(404).json({ success: false, message: error.message });
      else res.status(500).json({ success: false, message: error.message });
    }
